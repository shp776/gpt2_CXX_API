...
using namespace std;
static constexpr size_t BATCH_SIZE = 1; // batch_size 지정


// topk 알고리즘
vector<float> topKFrequent(vector<float> &a, int k)
{
...
size_t next_token_prediction(const std::unique_ptr<Ort::Session>& session_next, 
                             ...
                             float temperature,
                             int k_value,
                             vector<vector<float>>& initial_present_scores,
                             size_t present_score_data_num
                             ) {
                            //  float* outputs) {   // vector 받을때 &이용
    ...
    Ort::Value ids_tensor = Ort::Value::CreateTensor<int64_t>(memory_info_2, token_ids.data(), token_ids.size(), ids_tensor_shape.data(), ids_tensor_shape.size());
    
    vector<int64_t> prediction_scores_shape {BATCH_SIZE, static_cast<int64_t>(token_ids.size()), static_cast<int64_t>(vocab_size)};
    vector<float> prediction_scores(BATCH_SIZE * token_ids.size() * vocab_size);
    vector<int64_t> present_state_shape {2, BATCH_SIZE, 12, static_cast<int64_t>(token_ids.size()), 64};
    // vector<float> present_scores(2 * BATCH_SIZE * 12 * token_ids.size() * 64); // 0으로 초기화


    // save the present_scores and deliver to past_scores

    // vector<float> present_scores_0 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_1 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_2 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_3 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_4 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_5 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_6 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_7 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_8 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_9 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_10 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);
    // vector<float> present_scores_11 (2 * BATCH_SIZE * 12 * token_ids.size() * 64);

    // vector<vector<float>> past_scores(12, vector<float>(2 * BATCH_SIZE * 12 * token_ids.size() * 64)); // 2차원 벡터 초기화 선언
    vector<vector<float>> present_scores(12, vector<float>(present_score_data_num, 0)); 

    // name data main함수로부터 전달받아도 되지만 가시성을 위해
    vector<const char*> ort_input_name = {"input_ids", "past_0", "past_1", "past_2", "past_3", "past_4", "past_5", "past_6", "past_7", "past_8", "past_9", "past_10", "past_11"};
    vector<const char*> ort_output_name = {"logits", "present_0", "present_1", "present_2", "present_3", "present_4", "present_5", "present_6", "present_7", "present_8", "present_9", "present_10", "present_11" }; 

    vector<Ort::Value> ort_input_tensor;
    ort_input_tensor.push_back(move(ids_tensor)); 
  
    Ort::Value past_state_tensor_0 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[0].data(), initial_present_scores[0].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_1 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[1].data(), initial_present_scores[1].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_2 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[2].data(), initial_present_scores[2].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_3 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[3].data(), initial_present_scores[3].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_4 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[4].data(), initial_present_scores[4].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_5 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[5].data(), initial_present_scores[5].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_6 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[6].data(), initial_present_scores[6].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_7 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[7].data(), initial_present_scores[7].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_8 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[8].data(), initial_present_scores[8].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_9 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[9].data(), initial_present_scores[9].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_10 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[10].data(),initial_present_scores[10].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value past_state_tensor_11 = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[11].data(),initial_present_scores[11].size(), present_state_shape.data(), present_state_shape.size());  


    Ort::Value present_state_tensor_0 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[0].data(), present_scores[0].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_1 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[1].data(), present_scores[1].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_2 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[2].data(), present_scores[2].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_3 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[3].data(), present_scores[3].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_4 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[4].data(), present_scores[4].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_5 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[5].data(), present_scores[5].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_6 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[6].data(), present_scores[6].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_7 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[7].data(), present_scores[7].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_8 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[8].data(), present_scores[8].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_9 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[9].data(), present_scores[9].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_10 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[10].data(), present_scores[10].size(), present_state_shape.data(), present_state_shape.size());
    Ort::Value present_state_tensor_11 = Ort::Value::CreateTensor<float>(memory_info_2, present_scores[11].data(), present_scores[11].size(), present_state_shape.data(), present_state_shape.size());  

    if(loop==0) {
        ort_input_tensor.push_back(move(past_state_tensor_0));  
        ort_input_tensor.push_back(move(past_state_tensor_1));  
        ort_input_tensor.push_back(move(past_state_tensor_2));  
        ort_input_tensor.push_back(move(past_state_tensor_3));  
        ort_input_tensor.push_back(move(past_state_tensor_4));  
        ort_input_tensor.push_back(move(past_state_tensor_5));  
        ort_input_tensor.push_back(move(past_state_tensor_6));  
        ort_input_tensor.push_back(move(past_state_tensor_7));  
        ort_input_tensor.push_back(move(past_state_tensor_8));  
        ort_input_tensor.push_back(move(past_state_tensor_9));  
        ort_input_tensor.push_back(move(past_state_tensor_10));  
        ort_input_tensor.push_back(move(past_state_tensor_11));  

        // for (int n = 0; n < 12; n++) {
        //     // Ort::Value past_state_tensor = Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[n].data(), initial_present_scores[n].size(), present_state_shape.data(), present_state_shape.size());
        //     // ort_input_tensor.push_back(move(past_state_tensor));
        //     ort_input_tensor.push_back(
        //         Ort::Value::CreateTensor<float>(memory_info_2, initial_present_scores[n].data(), initial_present_scores[n].size(), present_state_shape.data(), present_state_shape.size()));

        // }
    }
   
    else {
        ort_input_tensor.push_back(move(present_state_tensor_0));  
        ort_input_tensor.push_back(move(present_state_tensor_1));  
        ort_input_tensor.push_back(move(present_state_tensor_2));  
        ort_input_tensor.push_back(move(present_state_tensor_3));  
        ort_input_tensor.push_back(move(present_state_tensor_4));  
        ort_input_tensor.push_back(move(present_state_tensor_5));  
        ort_input_tensor.push_back(move(present_state_tensor_6));  
        ort_input_tensor.push_back(move(present_state_tensor_7));  
        ort_input_tensor.push_back(move(present_state_tensor_8));  
        ort_input_tensor.push_back(move(present_state_tensor_9));  
        ort_input_tensor.push_back(move(present_state_tensor_10));  
        ort_input_tensor.push_back(move(present_state_tensor_11));  
        
        // Ort::Value present_state_tensor = Ort::Value::CreateTensor<float>(memory_info_2, present_scores_0.data(), present_scores_0.size(), present_state_shape.data(), present_state_shape.size()); 
        // ort_input_tensor.push_back(move(present_state_tensor));

    //     for (int n = 0; n < 12; n++) {
    //         ort_input_tensor.push_back(
    //             Ort::Value::CreateTensor<float>(memory_info_2, present_scores[n].data(), present_scores[n].size(), present_state_shape.data(), present_state_shape.size()));

    //     } 
    }
    size_t input_count = 13;
    ...
    ort_output_tensor.push_back(
        Ort::Value::CreateTensor(memory_info_2, prediction_scores.data(), prediction_scores.size(), prediction_scores_shape.data(), prediction_scores_shape.size()));


    // for (int n = 0; n < 12; n++){
    //     // Ort::Value present_state_tensor = Ort::Value::CreateTensor(memory_info_2, present_scores.data(), present_scores.size(), present_state_shape.data(), present_state_shape.size());   
    //     // ort_output_tensor.push_back(move(present_state_tensor)); // 0으로 초기화 되고 size와 dimension 잡아주고 0으로 초기화 된 preseent_state_tensor을 push_back  
    //     ort_output_tensor.push_back(
    //          Ort::Value::CreateTensor(memory_info_2, present_scores[n].data(), present_scores[n].size(), present_state_shape.data(), present_state_shape.size())); // 0으로 초기화 되고 size와 dimension 잡아주고 0으로 초기화 된 preseent_state_tensor을 push_back

    // }      

    ort_output_tensor.push_back(move(present_state_tensor_0));  
    ort_output_tensor.push_back(move(present_state_tensor_1));  
    ort_output_tensor.push_back(move(present_state_tensor_2));  
    ort_output_tensor.push_back(move(present_state_tensor_3));  
    ort_output_tensor.push_back(move(present_state_tensor_4));  
    ort_output_tensor.push_back(move(present_state_tensor_5));  
    ort_output_tensor.push_back(move(present_state_tensor_6));  
    ort_output_tensor.push_back(move(present_state_tensor_7));  
    ort_output_tensor.push_back(move(present_state_tensor_8));  
    ort_output_tensor.push_back(move(present_state_tensor_9));  
    ort_output_tensor.push_back(move(present_state_tensor_10));  
    ort_output_tensor.push_back(move(present_state_tensor_11));      
     

    size_t output_count = 13;                   
    ...
    session_next->Run(   // C_api.h의 Run과 같다. (즉, GetApi의 ORT_API2_STATUS(Run, ~~ ) 과 같다.)  사전 할당된 출력 목록이 있을 때 실행
    Ort::RunOptions{},
    ort_input_name.data(),
    ...
    // for checking session_Run output

    // float* ort_outputs[13];   
    // for (int i = 0; i < 13; i++)
    ...
    //   // present_scores(output[2]~[12])
    // for (int j = 0; j < 12; j++){    
    //     for (size_t i = 0; i < ? ; i++){
    //         past_scores[j][i] = outputs[j+1][i];
    //     }    
    ...
    // top_k_logits 함수 처리
    prediction_scores = topKFrequent(prediction_scores, k_value);
    // prediction_scores = topKFrequent(prediction_scores, 40);

    const auto new_word_prediction = view<float>{prediction_scores.begin() + (token_ids.size()-1) * vocab_size, prediction_scores.end()};                                                
    ...
    softmax(new_word_prediction);   // softmax후의 결과로 transform 된다.

    // sample: prev = torch.multinomial(num_samples=1)
...
//   // cxxopts -> option_parser end.
                    ...
                    //for debugging
  const std::string text = "Here is some text to encode Hello World";
  const size_t generate = 5;
  const float temperature = 1;
  const float k_value = 40;
  ...
  // SetInterOpNumThreads를 1이 아닌 다른 값으로 설정하면 CPU가 초과 구독되고 성능이 매우 나쁩니다.
  ...
  const wchar_t* model_path_2 = L"gpt2-lm-head-13dyn.onnx";
#else
  const char* model_path = "gpt2-lm-head-1-dyn_out.onnx";
  const char* model_path_2 = "gpt2-lm-head-13dyn.onnx";
  ...
  for (int i = 0; i < num_input_nodes; i++) {
    ...
    // else{
    // input_node_dims[1] = BATCH_SIZE;  //1    
    // input_node_dims[3] = token_ids.size();  // 사실 initial_past 값은 NULL 이므로 NULL 값이 맞다.      
    // }
  ...
  for (int i = 0; i < num_output_nodes; i++) {
    ...
    // output_node_dims[1] = token_ids.size();
...
//   vector<vector<float>> initial_present_scores;
  vector<vector<float>> initial_present_scores(12, vector<float>(present_score_data_num, 0));
  ...
  for (int j = 0; j < 12; j++){    
    ...
    for (size_t i = 0; i < present_score_data_num ; i++){
        ...
        initial_present_scores[j][i] = outputs[j+1][i];
        // initial_present_scores[j].push_back(outputs[j+1][i]);
    }
...
// for checking
//   FILE *fr = fopen("Final_CXX_save_initial_present_scores.txt", "w");
...
//     for (size_t i = 0; i < present_score_data_num ; i++){
//         // printf("output[%d] tensor [%lu] =  %f\n", j+2, i, outputs[j+1][i]);
//         fprintf(fr, "present_score(output[%d])[%lu] =  %f\n", j+1, i, initial_present_scores[j][i]);   
//     }
//   }      
//   fclose(fr);
...
//   Ort::Session session_next(env, model_path, session_options_next);
  auto session_next = std::make_unique<Ort::Session>(env, model_path_2, session_options_next);
  ...
  for (size_t i = 0; i < generate; ++i) {  // generate: ("n,number", "Number of new words to generate from initial text", cxxopts::value<size_t>()->default_value("1"))
    ...
    token_ids.push_back(
        // next_token_prediction(session_next, token_ids, tokenizer.vocab_size(), loop, temperature, k_value, output_tensors));
        next_token_prediction(session_next, token_ids, tokenizer.vocab_size(), loop, temperature, k_value, initial_present_scores, present_score_data_num));
        loop = loop + 1;
